...
$allMachines | ForEach-Object {
    ...
    $newJob = Start-ThreadJob -ScriptBlock {
        ...
        # Define the script content to stop the service
        $scriptContent = {
            param($serviceName)
            ...
            'Starting service stop process...' | Out-File -FilePath $logFile -Append
            ...
            try {
                ...
                if ($service.Status -eq 'Running') {
                    ...
                    'Service stopped successfully.' | Out-File -FilePath $logFile -Append
                } else {
                    'Service was not running.' | Out-File -FilePath $logFile -Append
                }
            } catch {
                'Error stopping service: ' + $_ | Out-File -FilePath $logFile -Append
            }
        ...
        try {
            Write-Output "Executing inline script to stop service on VM [$name]"
            ...
            # Invoke REST method to cancel the maintenance run
            Invoke-AzRestMethod -Path "/subscriptions/$subscriptionId/resourceGroups/$rg/providers/Microsoft.Maintenance/maintenanceConfigurations/$maintenanceRunId?api-version=2023-09-01-preview" -Payload @{
                properties = @{
                    status = "Cancel"
                }
            } -Method PUT
